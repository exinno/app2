...
export default function toSource(
  ...
  function walk(
    ...
    if (keys.length) {
      return `{${join(
        keys
          .filter((key) => object[key] !== undefined && (!ignoreStartsWith || !key.startsWith(ignoreStartsWith)))
          .map((key) => {
            const walked = walk(object[key], replacer, indent, nextIndent, seen.slice());
            if (typeof object[key] == 'function' && new RegExp(`^(async\\s*|)${key}`).test(walked)) {
              return walked;
            } else {
              return (legalKey(key) ? key : JSON.stringify(key)) + ':' + walked;
            }
          })
      )}}`;
    }
